...
export const createNewTicketCtr = async (req, res, next) => {
    ...
    try {
        ...
        const htmlText = `
            <p>Hello Team secretweb.vip,</p>
            <p>You got a new ticket message from ${req.username}:</p>
            <p>&nbsp;</p>
            <p style="padding: 12px; border-left: 4px solid #d0d0d0;">${message}</p>

            ${file ? 'attached file: <br><br> https://secretweb.vip/api/'+file : file}

            <p>&nbsp;</p>
            <p>Thanks for choosing secretweb.vip</p>
            <p>&nbsp;</p>
            <p><em>Best wishes,</em><br><strong>Team <span style="color: #de2341;">secretweb.vip</span></strong></p>
        `;

        const mailText = `
            Hello Team secretweb.vip,

            You got a new ticket message from ${req.username}:
            
            ${message}
            
            ${file ? 'https://secretweb.vip/api/'+file : file}
            
            Thanks for choosing secretweb.vip
        ...
        let details = {
            ...
            to: `contact@tesamedia.com`, // who should recieve the mail
            bcc: "sundaywht@gmail.com, support@secretweb.vip",
        ...
        const result = await general.createNewTicket(data2send);
...
export const addFundsCtr = async (req, res, next) => {
    ...
    try {
        ...
        let addFundsResponse = await user.addFunds(data2send);
